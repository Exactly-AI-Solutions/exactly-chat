...
- **Open items:** only `https://comm-fit-concierge.vercel.app` is whitelisted — the hosted `/demo` (origin `https://exactly-chat.vercel.app`) and any localhost dev origin are **not** yet enabled for the Comm-Fit key; add them with `set-config --origins` if wanted (the guide's §8 flags this). KB is v0.1 (public-facts only — no client-supplied pricing tiers, named case studies, or warranty terms yet).
